...
export const App = (): ReactElement => {
  ...
  const updateSourceMarkdown = (markdown: string) => {
    setSourceMarkdown(markdown)
    const [parsed] = parseMarkdown(markdown)
    if (!parsed) return
    const nextScenario = applyMarkerPositions(parsed, markerPositionStore)
    setScenario(nextScenario)
    setSelectedId(nextScenario.steps[0]?.id ?? '')
  }
