...
export function Popover({ label, children, disabled = false }: { label: string; children: ReactNode; disabled?: boolean }) {
  ...
  return <div className="shared-popover" ref={root}>
    <button type="button" ref={trigger} disabled={disabled} aria-haspopup="dialog" aria-expanded={open} aria-controls={open ? id : undefined} onClick={() => setOpen(!open)}>{label}</button>
    {open && <div id={id} ref={panel} role="dialog" aria-label={label} tabIndex={-1} className="shared-popover-panel" data-scroll="light">
