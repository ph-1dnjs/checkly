...
기존 `tests/app.spec.ts`는 localStorage fallback을 기대하지만 renderer의 기준 저장소는 main 프로세스의 `scenarios.md`입니다. 빈 원문·복제 테스트는 테스트 내부에서 electronAPI mock을 주입합니다. mock 기반 UI 검증과 실제 native 저장 검증을 구분합니다.
...
1. `useRunOrchestration.beginRuns`가 빈 배열을 차단하고 실행 대상 배열, sequence와 worker ID를 만듭니다.
2. 각 시나리오를 순서대로 `qa:start`에 전달합니다.
3. main은 worker ID가 같으면 하나의 headless Chromium과 BrowserContext를 재사용하고 시나리오마다 새 Page를 엽니다.
4. 각 Page는 선택 viewport(초기 2560×1440)를 적용하고 녹화 크기는 1280×720입니다.
...
- 입력: CSS는 value 속성 조건을 제거한 첫 요소, 일반 target은 label·placeholder·name 부분 일치의 결합 locator에서 첫 요소를 사용합니다.
- 클릭: CSS selector가 있으면 보이는 n번째 요소, 아니면 모든 frame의 button·label·텍스트 후보를 중복 제거하고 frame/DOM 순서로 정렬합니다.
- 선택: native `select`면 label/value로 `selectOption`; custom UI면 대상과 옵션을 각각 클릭합니다.
- 텍스트 확인: 대소문자 무시 정규식으로 보이는 텍스트가 나타날 때까지 반복합니다.
- 팝업: 새 page를 미리보기·직접 제어 대상으로 바꾸고 닫히면 원래 page로 돌아갑니다. 자동 액션·조건은 원래 page를 계속 사용합니다.
- 조건부 단계: 지정 텍스트가 제한 시간 안에 보이지 않으면 실패가 아니라 해당 단계를 건너뜁니다.

## 편집·실행 정책의 기준 문서

원문과 좌표는 독립적으로 저장하며 두 파일의 일관성을 보장하는 트랜잭션은 없습니다. 같은 실행 묶음은 BrowserContext를 공유하고, 최근 실행 기록·캡처는 메모리, 리포트·영상은 디스크에 남습니다. 상세 저장 순서·보존 범위·오류 제한은 [편집 저장 정책](../04-pages/020-scenario-editor/03-api.md), [실행 저장 정책](../04-pages/040-scenario-run/03-api.md), [실행 예외](../04-pages/040-scenario-run/04-edge-cases.md)를 기준으로 확인합니다.
